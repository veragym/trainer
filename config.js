...
// ============================================================
const SUPA_URL  = 'https://inioqgwyddhpaedcmpuq.supabase.co';
const SUPA_ANON = 'sb_publishable_PkcRSECy18SQ7J7eses57w_HyVMDnpj';
// ============================================================
...
/**
 * 로그인 체크 + 트레이너 정보 반환
 * 로그인 안 됐으면 trainer-login.html로 리다이렉트
 */
async function requireTrainer() {
  const { data: { session } } = await db.auth.getSession();
  if (!session) { location.replace('trainer-login.html'); return null; }
  const saved = localStorage.getItem('vg_trainer');
  if (!saved) { location.replace('trainer-login.html'); return null; }
